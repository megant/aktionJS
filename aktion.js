 ...
 * @attr data-aktion-type Action type (can be: set|toggle|add|remove, default: toggle)
 ...
 * on the destination DOM element (can be 'static'|'attribute'|'event')
...
var Aktion = function(customConfig) {
    ...
    var Helpers = {
        ...
        getOuterHeight: function (el) {
            if (el == window) return el.offsetHeight;
    ...
    var checkScrollEvent = function (index) {
        ...
        if (direction === null || element.scrollContainer.scrollTop < 0 || element.scrollContainer.scrollTop + Helpers.getOuterHeight(element.scrollContent) > Helpers.getDocumentDimension('height')) {
            return false;
        }
        ...
        switch (element.event) {
            case 'scroll-start':
                ...
                break;
            case 'scroll-end':
                condition = (element.lastY == element.scrollContainer.scrollTop);
                break;
            case 'scroll':
                condition = (null !== element.lastY && element.lastY != element.scrollContainer.scrollTop);
                break;
            case 'scroll-up':
                condition = (null !== element.lastY && element.lastY < element.scrollContainer.scrollTop);
                break;
            ...
            case 'scroll-dir-change':
                condition = (null !== element.lastY && ((element.direction != 1 && element.lastY < element.scrollContainer.scrollTop) || (element.direction != -1 && element.lastY > element.scrollContainer.scrollTop)));
                break;
            case 'scroll-dir-change-up':
                condition = (null !== element.lastY && element.direction != 1 && element.lastY < element.scrollContainer.scrollTop);
                break;
            case 'scroll-dir-change-down':
                condition = (null !== element.lastY && element.direction != -1 && element.lastY > element.scrollContainer.scrollTop);
                break;
            case 'scroll-reached-top':
                condition = (null !== element.lastY && element.direction != 1 && element.scrollContainer.scrollTop == 0);
                break;
            case 'scroll-reached-bottom':
                condition = (null !== element.lastY && element.direction != -1 && element.scrollContainer.scrollTop + element.scrollContainer.offsetHeight == element.scrollContent.offsetHeight);
                break;
    ...
    var triggerAction = function (element, _this) {
        ...
        if (element.value_type != 'event') {
            ...
            $((element.destination_selector === null) ? element.source_selector : element.destination_selector).trigger(element.value);
        }
    }
    ...
    var handleScrollActions = function(element) {
        ...
        element.scrollContainer = (element.source_selector == 'window' || element.source_selector == 'document') ? [window] : element.sourceElm;

        Helpers.forEach(element.scrollContainer, function(elm, idx) {
            ...
            elm.parentNode.on('touchstart mousedown', function (evt) {
            ...
            elm.parentNode.on('touchend touchcancel mouseup', function (evt) {
